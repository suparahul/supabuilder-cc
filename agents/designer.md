...
**explore**: 2-3 design variations as flow diagrams and HTML prototypes (see Prototyping > Variations for file organization). When the product has multiple user types, ensure variations address key differences in each type's journey. Pause for user to choose direction. Propose write plan.
...
**You own `product-wiki/ui-kit/`** — the product's visual language as reusable CSS. Init detects token source files; you do the actual extraction/creation during your first mission.

**Visual language for new products is significant creative work.** During explore mood for a new product's first mission, dedicate time to visual language exploration (2-3 visual directions) alongside screen variations — don't rush it as a side task. The visual language sets the tone for every future screen.
...
- UI Kit (`product-wiki/ui-kit/`) — design tokens, component styles, preview page

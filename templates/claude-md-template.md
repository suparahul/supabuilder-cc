...
| **Complete** | All building/refinement in the mission is done, user confirms finished.                             | Close out wikis, state, memory, UI Kit. Suggest follow-ups.                                                                                 | **Non-negotiable — do ALL:** 1) Update `product-wiki/` with History entry. 2) Update `code-wiki/` with History entry. 3) **Kit sync:** if the mission produced prototypes, spawn `designer` for a targeted kit sync (context: prototype paths, QA prototype-deviation notes, `product-wiki/ui-kit/` path) — promotes final screens to `ui-kit/screens/`, updates the `## Screens` registry. 4) **Always, every mission:** append to `product-wiki/ui-kit/README.md ## Sync Log`: `{date} — mission {id} ({type}) completed — designer: in pipeline \| not in pipeline`. 5) Move mission to `past_missions` in state.json, set `status: done`. 6) Update `memory.md` with mission summary. 7) Suggest follow-up mission if natural continuation exists.                                    |
...
{ "mission": { "id": "", "type": "", "phase": "", "decisions": []},
  ...
  "file_paths": ["specs, prototypes, code files only for architect or qa"],
  "designer_extras": { "ui_kit_path": "product-wiki/ui-kit/", "ui_kit_status": "populated | empty",
    "last_synced": "last entry from ui-kit README ## Sync Log, or null" } }
```

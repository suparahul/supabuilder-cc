...
description: "Configure Supabuilder settings — cost mode, user control, orchestrator on/off."
user-invocable: true
argument-hint: [quality|smart|budget|hands-on|guided|autonomous|on|off|show]
---

# Supabuilder Settings

Configure how Supabuilder operates. All settings are stored in `supabuilder/settings.json`.
...
|---------|--------|---------|-----------------|
| `cost_mode` | `quality` / `smart` / `budget` | `smart` | Which models agents use |
...
1. **Read** `supabuilder/settings.json`
...
{new} mode:
  quality  — Opus for all agents (~3-5x cost)
  smart    — Opus for PM/Designer/Strategist/Architect, Sonnet for TechPM/QA
  budget   — Sonnet for all except critical (~50% cost)
```
